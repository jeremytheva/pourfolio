...
if (nvmrc !== null && nvmrc.trim() !== expectedMajor) {
  findings.push({ code: 'NVMRC_RUNTIME_MISMATCH', expected: expectedMajor, actual: nvmrc.trim() })
}
...
if (packageManifest && packageManifest.engines?.node !== expectedEngine) {
  findings.push({ code: 'PACKAGE_NODE_ENGINE_MISMATCH', expected: expectedEngine, actual: packageManifest.engines?.node ?? null })
}
...
for (const relativePath of activeGuidance) {
  ...
  if (content !== null && !content.includes(`Node.js ${expectedMajor}`)) {
    findings.push({ code: 'RUNTIME_GUIDANCE_MISMATCH', path: relativePath, expected: `Node.js ${expectedMajor}` })
  }
}
...
if (validationWorkflow !== null) {
  ...
  if (setupNodeReferences.length < 2) {
    findings.push({ code: 'CI_RUNTIME_CONTRACT_DIVERGED', path: validationWorkflowPath, expected: 'both source validation and browser validation use .nvmrc' })
  }
}
