...
- PR #252 (`frontend/cellar-load-recovery`) is the current independent frontend slice: focus the owner-scoped cellar load failure and prove failure → retry → recovered list behaviour with Playwright coverage.
...
1. consume PR #252 exact-head Pull request validation, Browser/accessibility, Dependency Review and CodeQL evidence and repair any failure without weakening the contract;
