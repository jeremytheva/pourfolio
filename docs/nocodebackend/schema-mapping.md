...
`id` or `user_id` must match the immutable authenticated user ID. Editable browser fields are limited to `name`, `description`, and `avatar_url`. Email, role, identity and provider metadata are never accepted from a profile update.
...
| `user_id` | Set by the server from the authenticated session. |
| `rating_id` | Positive safe-integer client submission ID; unique per owner and used for durable retry idempotency. |
| `product_id` | Required existing `products.id`. |
| `cellar_id` | Optional owner-held `cellar.id` for the same product. |
| `date_rated` | Server timestamp. |
...
Each applicable scored attribute must occur exactly once per rating. `attribute_score` is an integer from 1 through 7 inclusive. Score `1` is valid and must not be treated as missing. `user_id` is set by the server.

### `bonus_attribute_rating_mapping`

Bonus selections are optional. Each submitted `bonus_attributes_id` must exist. `user_id` and `rating_id` are set by the server.
...
6. Return an existing owner record when the same `rating_id` is retried.
...
There is no executable migration runner in this repository. Validate collection paths, fields, filters, permission rules, create response shapes and rollback behaviour in a non-production instance. Rehearse rollback and backup restore before switching production traffic.
