...
The supplied `bonus_attributes` CSV contains many historical rows whose `point_value` is blank. This feature does not mutate those source rows. A blank historical value receives an effective value of `0.2` when used in rating calculation; an existing stored numeric value is preserved.
...
After the normal rating cards, the rating flow presents an **All bonus attributes** step. It provides:
...
The scored Bonus dimension is derived as follows:
...
This preserves the requested behaviour that both totals below 1 and totals from 1 up to (but not including) 2 produce Bonus 1.

The browser displays a live preview, but this value is not authoritative. On submission the server:
...
4. derives Bonus 0/1/2;
5. replaces any client-supplied Bonus score with the derived score before calculating authoritative rating totals;
6. stores the selected IDs in `bonus_attribute_rating_mapping`.
