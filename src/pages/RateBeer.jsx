...
function RateBeer() {
  ...
  const retailPPP = preview && selectedCellar
    ? calculatePPP(preview.total_weighted, selectedCellar.retail_price, selectedCellar.mls)
    : null
  const purchasedPPP = preview && selectedCellar
      ...
      <div className="mx-auto max-w-3xl px-4 py-16">
        <div ref={errorRef} tabIndex={-1} role="alert" className="rounded-xl border border-red-200 bg-red-50 p-6 text-red-900 outline-none focus:ring-2 focus:ring-red-300">
          ...
          <button type="button" onClick={retryLoad} className="mt-4 inline-flex items-center rounded-lg bg-red-700 px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-red-300 focus:ring-offset-2">
            <SafeIcon icon={FiRefreshCw} className="mr-2 h-4 w-4" />Try again
          </button>
        </div>
      </div>
    )
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      <Link to={`/products/${productId}`} className="mb-6 inline-flex items-center rounded text-sm font-medium text-gray-600 hover:text-amber-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2">
        <SafeIcon icon={FiArrowLeft} className="mr-2 h-4 w-4" />Back to product
      </Link>

      <header className="mb-6">
        <p className="text-sm font-semibold uppercase tracking-wide text-amber-700">Structured beer rating</p>
        <h1 ref={headingRef} tabIndex={-1} className="mt-2 text-3xl font-bold text-gray-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2">{product.product_name}</h1>
      ...
      <div className="mb-6" aria-label={`Rating progress: step ${step + 1} of ${totalSteps}`}>
        <div className="mb-2 flex justify-between text-sm text-gray-600"><span>Step {step + 1} of {totalSteps}</span><span>{progress}%</span></div>
        <div className="h-2 overflow-hidden rounded-full bg-gray-200" aria-hidden="true">
          <div className="h-full rounded-full bg-amber-600 transition-[width] motion-reduce:transition-none" style={{ width: `${progress}%` }} />
        </div>
      </div>
      ...
      <form onSubmit={submit} aria-busy={submitting ? 'true' : 'false'}>
        ...
        <section
          ...
          aria-describedby="rating-required-help"
        ...
        >
          {currentRow && (() => {
            ...
            const helpId = `score-${attribute.id}-weight`
            return (
              <div>
                ...
                <p id={helpId} className="mt-2 text-sm text-gray-600">
                  {dimension.scored
                    ? (weight > 0 ? `Your weight: ${Math.round(weight * 100)}%` : 'Your weight is 0% — rate this only if you want to.')
                    : 'This does not change your Pourfolio score.'}
                </p>
                <div className="mt-8 rounded-2xl bg-amber-50 p-5">
                  <div className="text-center" aria-live="polite" aria-atomic="true">
                    ...
                    <div className="mt-1 text-5xl font-bold text-amber-950">{scores[attribute.id] ?? '—'} <span className="text-2xl font-medium text-amber-800">/ {dimension.max}</span></div>
                  </div>
                  ...
                  <input
                    ...
                    max={dimension.max}
                    step="1"
                    value={scores[attribute.id] ?? min}
                    onChange={(event) => selectScore(attribute.id, event.target.value)}
                    onPointerUp={(event) => {
                      event.stopPropagation()
                      selectScore(attribute.id, event.currentTarget.value, true)
                    }}
                    className="mt-4 w-full accent-amber-700"
                    aria-describedby={`${helpId} rating-required-help`}
                    aria-valuetext={`${scores[attribute.id] ?? min} out of ${dimension.max}`}
                  />
                  <div className={`mt-5 grid gap-2 ${optionGrid}`} aria-label={`${dimension.label} quick score selection`}>
                    {options.map((score) => (
                      <button
                        key={score}
                        type="button"
                        aria-pressed={scores[attribute.id] === score}
                        aria-label={`${dimension.label}: ${score} out of ${dimension.max}`}
                        onClick={() => selectScore(attribute.id, score, true)}
                        className={`min-h-11 rounded-xl border text-sm font-semibold focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2 ${scores[attribute.id] === score ? 'border-amber-700 bg-amber-700 text-white' : 'border-gray-300 bg-white text-gray-800 hover:bg-amber-50'}`}
                      >{score}</button>
                    ))}
                  </div>
                </div>
            ...
            <div>
              ...
              <div className="mt-6 divide-y divide-gray-200 rounded-xl border border-gray-200">
                {rows.map(({ attribute, dimension, weight }, index) => (
                  <div key={attribute.id} className="flex items-center justify-between gap-3 p-4">
                    <div>
                      <p className="font-medium text-gray-900">{dimension.label}</p>
                      <p className="text-xs text-gray-500">{dimension.scored ? `${Math.round(weight * 100)}% weight` : 'Fun extra'}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <strong className="text-amber-800">{scores[attribute.id] === undefined ? 'Skipped' : `${scores[attribute.id]} / ${dimension.max}`}</strong>
                      <button type="button" onClick={() => moveToStep(index)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2">Edit</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {cellarItems.length > 0 && (
                <div className="mt-6">
                  <label htmlFor="rating-cellar" className="block text-sm font-semibold text-gray-800">Price source for PPP (optional)</label>
                  <select id="rating-cellar" value={cellarId} onChange={(event) => setCellarId(event.target.value)} className="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2">
                    <option value="">No cellar purchase selected</option>
                    {cellarItems.map((item) => (
                      <option key={item.id} value={item.id}>{item.mls ? `${item.mls} mL` : 'Unknown volume'} · purchased {item.purchase_price ? `$${item.purchase_price}` : 'price unavailable'} · retail {item.retail_price ? `$${item.retail_price}` : 'price unavailable'}</option>
                    ))}
                  </select>
                  <p className="mt-1 text-xs text-gray-500">PPP is only calculated when a price and container volume are available.</p>
                </div>
              )}
              ...
              <section className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5" role="status" aria-live="polite" aria-atomic="true">
                ...
                {preview ? (
                  <dl className="mt-3 grid gap-4 sm:grid-cols-2">
                    <div><dt className="text-sm text-amber-800">Personalised score</dt><dd className="text-2xl font-bold text-amber-950">{preview.total_weighted.toFixed(2)} / 5</dd></div>
                    <div><dt className="text-sm text-amber-800">Score out of 100</dt><dd className="text-2xl font-bold text-amber-950">{preview.score_out_of_100.toFixed(2)}</dd></div>
                    <div><dt className="text-sm text-amber-800">Default-weight score</dt><dd className="text-xl font-semibold text-amber-950">{preview.total_unweighted === null ? 'Needs all default attributes' : `${preview.total_unweighted.toFixed(2)} / 5`}</dd></div>
                    <div><dt className="text-sm text-amber-800">Overall Scaled Score</dt><dd className="text-xl font-semibold text-amber-950">Calculated after submission</dd></div>
              ...
              <button type="submit" disabled={submitting || !preview} aria-busy={submitting} className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-amber-700 px-5 py-3 font-semibold text-white hover:bg-amber-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2 disabled:cursor-wait disabled:bg-gray-500">
                <SafeIcon icon={FiCheck} className="mr-2 h-5 w-5" />{submitting ? 'Submitting securely…' : 'Submit rating'}
              </button>
            </div>
          )}
        </section>
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          <button type="button" onClick={() => moveToStep(step - 1)} disabled={step === 0 || submitting} className="inline-flex min-w-28 items-center justify-center rounded-xl border border-gray-300 bg-white px-4 py-3 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-40">
            <SafeIcon icon={FiChevronLeft} className="mr-1 h-5 w-5" />Back
          </button>
          {!isReview && (
            <button type="button" onClick={() => moveToStep(step + 1)} disabled={!canAdvance || submitting} className="inline-flex min-w-28 items-center justify-center rounded-xl bg-gray-900 px-4 py-3 font-medium text-white hover:bg-gray-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:bg-gray-400">
              Next<SafeIcon icon={FiChevronRight} className="ml-1 h-5 w-5" />
            </button>
          )}
        </div>
      </form>
