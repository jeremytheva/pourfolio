...
function RateBeer() {
  ...
  const preview = useMemo(() => {
    ...
    try {
      return calculateRatingTotals(
        formDefinition.attributes.map((attribute) => ({ attributeId: attribute.id, score: scores[attribute.id] })),
  ...
  const submit = async (event) => {
    ...
    try {
      await ratingService.submitRating({
        productId,
        submissionId,
        scores: formDefinition.attributes.map((attribute) => ({ attributeId: attribute.id, score: scores[attribute.id] })),
  ...
  const canAdvance = !currentAttribute || scores[currentAttribute.id] !== undefined
  ...
  const selectScore = (attributeId, value, autoAdvance = false) => {
    ...
    setScores((current) => ({ ...current, [attributeId]: score }))
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-6" aria-label={`Rating progress: step ${step + 1} of ${totalSteps}`}>
        <div className="mb-2 flex items-center justify-between text-sm text-gray-600"><span>Step {step + 1} of {totalSteps}</span><span>{progressValue}%</span></div>
      ...
      {error && <div ref={errorRef} tabIndex={-1} className="mb-6 rounded-lg border border-red-200 bg-red-50 p-4 text-red-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-2" role="alert">{error}</div>}

      <form onSubmit={submit} aria-busy={submitting}>
        <p id="rating-required-help" className="sr-only">Every applicable attribute is required. A score of 1 is valid and is not treated as missing.</p>
        <section
          role="group"
        ...
        >
          {currentAttribute && (
            <div>
              ...
              <p id={`score-${currentAttribute.id}-weight`} className="mt-2 text-sm text-gray-500">Weight {currentAttribute.weighting}. Choose a score from 1 to 7.</p>
              ...
              <div className="mt-10 rounded-2xl bg-amber-50 p-5 sm:p-6">
                ...
                <label htmlFor={`score-${currentAttribute.id}`} className="mt-8 block text-sm font-semibold text-gray-800">{currentAttribute.attribute_name} score</label>
                <input
                  ...
                  aria-describedby={`score-${currentAttribute.id}-weight rating-required-help`}
                  ...
                  onPointerUp={(event) => {
                    ...
                    selectScore(currentAttribute.id, event.currentTarget.value, true)
                  }}
                  className="mt-4 w-full accent-amber-700"
                />
                <div className="mt-5 grid grid-cols-7 gap-2" aria-label={`${currentAttribute.attribute_name} quick score selection`}>
                  {scoreOptions.map((score) => (
                    <button
                      key={score}
                      type="button"
                      aria-pressed={scores[currentAttribute.id] === score}
                      aria-label={`${currentAttribute.attribute_name}: ${score} out of 7`}
                      onClick={() => selectScore(currentAttribute.id, score, true)}
                      className={`min-h-11 rounded-xl border text-sm font-semibold focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2 ${scores[currentAttribute.id] === score ? 'border-amber-700 bg-amber-700 text-white' : 'border-gray-300 bg-white text-gray-800 hover:border-amber-400 hover:bg-amber-50'}`}
                    >{score}</button>
                  ))}
                </div>
                <div className="mt-2 flex justify-between text-xs text-gray-500" aria-hidden="true"><span>Low</span><span>High</span></div>
              </div>
              <p className="mt-6 text-center text-xs text-gray-500">Select a score to advance automatically, or swipe between completed cards.</p>
            </div>
          )}
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {formDefinition.bonusAttributes.map((bonus) => (
                  <label key={bonus.id} className="flex cursor-pointer items-start gap-3 rounded-xl border border-gray-200 p-4 hover:bg-gray-50 focus-within:ring-2 focus-within:ring-amber-700 focus-within:ring-offset-2">
                    ...
                    <span><span className="block font-medium text-gray-800">{bonus.description}</span>{bonus.point_value !== null && bonus.point_value !== undefined && <span className="text-sm text-gray-500">{bonus.point_value > 0 ? '+' : ''}{bonus.point_value}</span>}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          {isReview && (
            <div>
              ...
              <div className="mt-7 divide-y divide-gray-200 rounded-2xl border border-gray-200">
                {attributes.map((attribute, index) => (
                  <div key={attribute.id} className="flex items-center justify-between gap-4 p-4">
                    <div><div className="font-medium text-gray-900">{attribute.attribute_name}</div><div className="text-sm text-gray-500">Weight {attribute.weighting}</div></div>
                    <div className="flex items-center gap-3"><strong className="text-xl text-amber-800">{scores[attribute.id] ?? '—'} / 7</strong><button type="button" onClick={() => moveToStep(index)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2">Edit</button></div>
                  </div>
                ))}
              </div>
              <section className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5" role="status" aria-live="polite" aria-atomic="true">
                ...
                {preview ? <div className="mt-2 flex flex-wrap gap-6"><p><span className="text-sm text-amber-800">Weighted</span><br /><strong className="text-2xl text-amber-950">{preview.total_weighted} / 7</strong></p><p><span className="text-sm text-amber-800">Unweighted</span><br /><strong className="text-2xl text-amber-950">{preview.total_unweighted} / 7</strong></p></div> : <p className="mt-1 text-sm text-amber-900">Every attribute needs a score before submission.</p>}
              </section>
