...
function RateBeer({selectedBeverageCategory='beer'}) {
  ...
  useEffect(()=> {
    ...
    setHideBonusPoints(false); // Reset hide bonus points
  },[beverageType]);

  const handleMainAttributeChange=(attribute,value)=> {
    setMainAttributes(prev=> ({
      ...prev,
      [attribute]: {...prev[attribute],score: value}
    }));
  };
  ...
  const calculateCurrentRating=()=> {
    return calculateFinalRating(
      ...
      userBonusOverride,
      false, // Always calculate bonus for internal use
      beverageType
    );
  };
          ...
          {currentStep===1 && (
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold text-gray-800">Bonus Attributes</h2>
                
                {/* Hide Bonus Points Toggle - Moved here */}
                <div className="flex items-center space-x-2">
                  <button
                    type="button"
                    onClick={()=> setHideBonusPoints(!hideBonusPoints)}
                    className="flex items-center space-x-2 text-sm text-gray-600 hover:text-amber-600 transition-colors px-3 py-2 rounded-lg border border-gray-300 hover:border-amber-300"
                  >
                    <SafeIcon icon={hideBonusPoints ? FiEyeOff : FiEye} className="w-4 h-4" />
                    <span>{hideBonusPoints ? 'Show' : 'Hide'} Point Values</span>
                  </button>
                </div>
              </div>
