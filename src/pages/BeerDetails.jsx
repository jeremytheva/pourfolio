...
import { FiArrowLeft, FiEdit3, FiPackage, FiRefreshCw, FiStar, FiX } from 'react-icons/fi'
...
const CELLAR_CONTAINER_OPTIONS = ['Can', 'Bottle', 'Growler', 'Keg']
...
function BeerDetails() {
  ...
  const updateCellarContainerMode = (event) => {
    const mode = event.target.value
    setCellarContainerMode(mode)
    setCellarForm((current) => ({
      ...current,
      container: mode === 'other' ? '' : mode
    }))
    if (cellarStatus === 'saved') setCellarStatus('')
  }
