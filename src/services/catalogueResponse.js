...
const invalid = () => { throw new InvalidCatalogueResponse() }
...
const validateProducer = (value) => {
  ...
  const result = { id: validateStableId(producer.id), producer_name: validateText(producer.producer_name, { required: true }) }
  if (Object.hasOwn(producer, 'address')) result.address = validateText(producer.address, { maxLength: 1000 })
  if (Object.hasOwn(producer, 'suburb_id')) result.suburb_id = validateStableId(producer.suburb_id, { nullable: true, emptyAsNull: true })
  return result
}
const validateCategory = (value) => {
  if (value === null) return null
  const category = readDataProperties(value, CATEGORY_KEYS, ['id', 'category_name'])
  const result = { id: validateStableId(category.id), category_name: validateText(category.category_name, { required: true }) }
  if (Object.hasOwn(category, 'parent_id')) result.parent_id = validateStableId(category.parent_id, { nullable: true, emptyAsNull: true })
  return result
}
...
const validateProduct = (value, { detail = false } = {}) => {
  ...
  if (Object.hasOwn(product, 'product_category_id')) result.product_category_id = validateStableId(product.product_category_id, { nullable: true, emptyAsNull: true })
  if (Object.hasOwn(product, 'producer_id')) result.producer_id = validateStableId(product.producer_id, { nullable: true, emptyAsNull: true })
  ...
  if (Object.hasOwn(product, 'collaboration')) result.collaboration = validateOptionalDisplayScalar(product.collaboration)
  ...
  if (result.category && (!Object.hasOwn(result, 'product_category_id') || result.product_category_id === null || !sameId(result.category.id, result.product_category_id))) invalid()
  if (detail) {
...
export const validateCataloguePage = (payload, { expectedPage, expectedPageSize } = {}) => validate(() => {
  ...
  for (const value of [page.page, page.pageSize, page.total, page.totalPages]) if (!Number.isSafeInteger(value)) invalid()
