...
const invalid = () => { throw new ApiError(INVALID_PUBLIC_PROFILE_MESSAGE, { code: 'invalid_public_profile_response' }) }
...
const validateRating = (rating) => {
  ...
  if (rating.total_unweighted !== null && rating.total_unweighted !== undefined && !finiteScore(rating.total_unweighted)) invalid()
...
export const normalisePublicProfileId = (value) => {
  ...
  if (!PUBLIC_PROFILE_ID_PATTERN.test(id)) throw new ApiError('Profile identifier is invalid.', { status: 400, code: 'invalid_profile_identifier' })
  return id
}
