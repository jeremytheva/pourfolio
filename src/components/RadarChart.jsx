...
function RadarChart({ data, beverageType = 'beer', chartType = 'rating', width = '100%', height = '400px', showValue = true, compareData = null, className = '' }) {
  ...
  const initChart = () => {
    ...
    if (data.values && data.values.length > 0) {
      datasets.push({ name: data.name || `${currentBeverage.name} Rating`, value: data.values, itemStyle: { color: data.color || '#F59E0B' }, areaStyle: { color: data.color ? `${data.color}20` : '#F59E0B20' } });
    }
    if (compareData && compareData.values && compareData.values.length > 0) {
      datasets.push({ name: compareData.name || 'Comparison', value: compareData.values, itemStyle: { color: compareData.color || '#3B82F6' }, areaStyle: { color: compareData.color ? `${compareData.color}20` : '#3B82F620' } });
    }

    const option = {
      title: { text: `${currentBeverage.name} ${chartType === 'rating' ? 'Rating' : 'Characteristics'} Profile`, left: 'center', top: 20, textStyle: { color: '#374151', fontSize: 16, fontWeight: 'bold' } },
      legend: { data: datasets.map(d => d.name), bottom: 10, textStyle: { color: '#6B7280' } },
      radar: {
        indicator: config.indicators.map(ind => ({ name: ind.name, max: ind.max, color: ind.color })),
        shape: 'polygon', splitNumber: 4, radius: '65%', center: ['50%', '55%'],
        name: { textStyle: { color: '#374151', fontSize: 12, fontWeight: 'bold' } },
        splitArea: { areaStyle: { color: ['#FAFAFA', '#F3F4F6', '#E5E7EB', '#D1D5DB'], shadowColor: 'rgba(0,0,0,0.1)', shadowBlur: 5 } },
        splitLine: { lineStyle: { color: '#9CA3AF', width: 1 } },
        axisLine: { lineStyle: { color: '#6B7280', width: 2 } }
      },
      series: [{ name: `${currentBeverage.name} Analysis`, type: 'radar', data: datasets, symbol: 'circle', symbolSize: 6, lineStyle: { width: 3 }, emphasis: { itemStyle: { shadowBlur: 10, shadowColor: 'rgba(0,0,0,0.3)' } } }],
      tooltip: {
        trigger: 'item',
        formatter: function (params) {
          ...
          params.value.forEach((value, index) => {
            ...
            tooltip += `<div style="margin: 4px 0;display: flex;align-items: center;"> <span style="display: inline-block;width: 10px;height: 10px;background: ${indicator.color};margin-right: 8px;border-radius: 2px;"></span> <span style="flex: 1;">${indicator.name}:</span> <span style="font-weight: bold;margin-left: 8px;">${value}/${indicator.max} (${percentage}%)</span> </div>`;
          });
          tooltip += '</div>';
          return tooltip;
        },
        backgroundColor: 'rgba(255,255,255,0.95)', borderColor: '#E5E7EB', borderWidth: 1, textStyle: { color: '#374151', fontSize: 12 }
      },
    ...
    >
      ...
      {showValue && data.overallScore && (
        <div className="mt-4 text-center">
          <div className="text-2xl font-bold text-amber-600">{data.overallScore}/5</div>
