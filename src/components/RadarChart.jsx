...
function RadarChart({ 
  ...
  const echartsModuleRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const renderChart = async () => {
      ...
      if (!echartsModuleRef.current) {
        ...
        if (cancelled) return;
        echartsModuleRef.current = module.default ?? module;
      }

      if (!echartsModuleRef.current) return;

      if (!chartInstance.current) {
        chartInstance.current = echartsModuleRef.current.init(chartRef.current);
      }
