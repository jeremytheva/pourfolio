...
const fail = (message, status = 400, code = null, currentVersion = null) => {
  ...
  if (code) error.payload = { error: message, code, ...(currentVersion === null ? {} : { currentVersion }) }
  return error
}
...
const versionConflict = (record) => fail('The game changed before this request was applied.', 409, 'VERSION_CONFLICT', Number(record?.version || 0))
const idempotencyConflict = () => fail('This request key has already been used for different Brew Done It challenge data.', 409, 'IDEMPOTENCY_CONFLICT')
const compareAndSet = async (collection, record, expectedVersion, updates) => {
  ...
  try { await dataProvider.compareAndSet(collection, record.id, expectedVersion, { ...updates, version: expectedVersion + 1 }) }
  catch (error) {
...
const creationRequestFingerprint = (creationKey, productId) => crypto.createHmac('sha256', signingKey()).update(`create:${creationKey}:${productId}`).digest('hex')
const roundsFor = async (gameId) => list(await listAllBrewDoneItRecords(COLLECTIONS.brewDoneItRounds, { game_id: gameId })).sort((left, right) => Number(left.round_number || 0) - Number(right.round_number || 0))

const initialRoundBody = (game, productId, createdAt) => ({
  game_id: game.id, round_number: 1, selector_participant_id: game.creator_participant_id,
  guesser_participant_id: game.opponent_participant_id ?? null, selected_product_id: productId,
  status: game.opponent_participant_id ? 'guessing' : 'waiting_for_opponent', turn_sequence: 0,
  max_turns: BREW_DONE_IT_RULES.maxTurns, incorrect_guess_count: 0, incorrect_formal_guess_count: 0,
  brewery_correct: false, style_correct: false, beer_correct: false, pending_action_key: null,
  pending_action_type: null, pending_action_started_at: null, last_action_key: null, last_action_type: null,
  created_at: createdAt, started_at: game.opponent_participant_id ? game.joined_at || createdAt : null,
  completed_at: null, completion_reason: null, scoring_rules_version: null, awarded_points: null,
  score_breakdown: null, version: 0
})
const ensureInitialRound = async (game, productId, createdAt) => {
  const findPersisted = async () => list(await dataProvider.list(COLLECTIONS.brewDoneItRounds, { game_id: game.id, round_number: 1 }))[0] || null
  const existing = await findPersisted()
  if (existing) { if (String(existing.selected_product_id) !== String(productId)) throw idempotencyConflict(); return existing }
  try {
...
const gameByCreationKey = async (creationKey) => list(await dataProvider.list(COLLECTIONS.brewDoneItGames, { creation_idempotency_key: creationKey }))[0] || null
export const createGameV3 = async (request, response, user) => {
  ...
  if (game) {
    ...
    if (!game.creation_request_fingerprint && (!openingRound || String(openingRound.selected_product_id) !== String(productId))) throw idempotencyConflict()
    const round = openingRound || await ensureInitialRound(game, productId, game.created_at || new Date().toISOString())
    response.status(200).json({ game: projectBrewDoneItGame(game), round: projectBrewDoneItRound(round, user.id), invitationCode, replayed: true })
    return
  }
  const now = new Date().toISOString()
  const gameBody = {
    creator_participant_id: user.id, opponent_participant_id: null, creator_history_clues_enabled: false,
    opponent_history_clues_enabled: false, invitation_digest: invitationDigest(invitationCode), status: 'waiting',
    current_round_number: 1, version: 0, creation_idempotency_key: creationKey,
    creation_request_fingerprint: fingerprint, expires_at: new Date(Date.now() + INVITATION_LIFETIME_MS).toISOString(),
    created_at: now, joined_at: null, last_activity_at: now, archived_at: null
  }
  try { game = first(await dataProvider.create(COLLECTIONS.brewDoneItGames, gameBody)) }
  catch (error) { game = await gameByCreationKey(creationKey); if (!game) throw error }
...
export const joinGameV3 = async (gameId, request, response, user) => {
  ...
  if (game?.join_idempotency_key === requestKey && String(game.opponent_participant_id) === String(user.id)) {
    ...
    if (String(round.guesser_participant_id || '') !== String(user.id) || round.status === 'waiting_for_opponent') {
      try { persistedRound = await compareAndSet(COLLECTIONS.brewDoneItRounds, round, Number(round.version || 0), { guesser_participant_id: user.id, status: 'guessing', started_at: round.started_at || game.joined_at || new Date().toISOString() }) }
      catch (error) { const refreshed = await dataProvider.get(COLLECTIONS.brewDoneItRounds, round.id); if (String(refreshed?.guesser_participant_id) !== String(user.id) || refreshed?.status !== 'guessing') throw error; persistedRound = refreshed }
    }
    response.status(200).json({ game: projectBrewDoneItGame(game), round: projectBrewDoneItRound(persistedRound, user.id), replayed: true }); return
  }
  ...
  const joinedAt = new Date().toISOString(); const joinVersion = Number(game.version || 0)
  try { game = await compareAndSet(COLLECTIONS.brewDoneItGames, game, joinVersion, { opponent_participant_id: user.id, status: 'active', joined_at: joinedAt, last_activity_at: joinedAt, invitation_digest: digest, join_idempotency_key: requestKey }) }
  catch (error) { const refreshed = await dataProvider.get(COLLECTIONS.brewDoneItGames, id); if (refreshed?.join_idempotency_key !== requestKey || String(refreshed?.opponent_participant_id) !== String(user.id) || refreshed?.invitation_digest !== digest) throw error; game = refreshed }
  const round = (await roundsFor(game.id)).find((item) => Number(item.round_number) === 1)
  if (!round) throw fail('The opening round is unavailable.', 409)
  let persistedRound
  try { persistedRound = await compareAndSet(COLLECTIONS.brewDoneItRounds, round, Number(round.version || 0), { guesser_participant_id: user.id, status: 'guessing', started_at: round.started_at || joinedAt }) }
  catch (error) { const refreshed = await dataProvider.get(COLLECTIONS.brewDoneItRounds, round.id); if (String(refreshed?.guesser_participant_id) !== String(user.id) || refreshed?.status !== 'guessing') throw error; persistedRound = refreshed }
...
const nextRoundBody = ({ game, previous, productId, requestKey, now }) => ({
  game_id: game.id, round_number: Number(previous.round_number) + 1, selector_participant_id: previous.guesser_participant_id,
  guesser_participant_id: previous.selector_participant_id, selected_product_id: productId, status: 'guessing', turn_sequence: 0,
  max_turns: BREW_DONE_IT_RULES.maxTurns, incorrect_guess_count: 0, incorrect_formal_guess_count: 0,
  brewery_correct: false, style_correct: false, beer_correct: false, pending_action_key: null, pending_action_type: null,
  pending_action_started_at: null, last_action_key: null, last_action_type: null, created_at: now, started_at: now,
  completed_at: null, completion_reason: null, scoring_rules_version: null, awarded_points: null, score_breakdown: null,
  round_creation_idempotency_key: requestKey, version: 0
})
const canAdoptPersistedRound = (game, rounds, created, userId) => {
  const targetNumber = Number(created?.round_number || 0); const predecessorNumber = targetNumber - 1
...
const adoptPersistedRound = async (game, rounds, created, userId) => {
  ...
  const updates = { current_round_number: targetNumber, last_activity_at: created.created_at || new Date().toISOString() }
  try { return await compareAndSet(COLLECTIONS.brewDoneItGames, game, Number(game.version || 0), updates) }
  catch (error) {
...
export const createNextRoundV3 = async (gameId, request, response, user) => {
  ...
  if (replayed) {
    ...
    response.status(200).json({ game: projectBrewDoneItGame(game), round: projectBrewDoneItRound(replayed, user.id), replayed: true }); return
  }
  ...
  const now = new Date().toISOString(); const body = nextRoundBody({ game, previous, productId, requestKey, now }); let created
  try { created = first(await dataProvider.create(COLLECTIONS.brewDoneItRounds, body)) }
  catch (error) { rounds = await roundsFor(game.id); const persisted = rounds.find((round) => round.round_creation_idempotency_key === requestKey); if (!persisted) throw error; if (String(persisted.selected_product_id) !== String(productId)) throw idempotencyConflict(); created = persisted }
  if (!created?.id) throw fail('The game service did not return a round identifier.', 502)
  rounds = [...rounds.filter((round) => String(round.id) !== String(created.id)), created].sort((left, right) => Number(left.round_number || 0) - Number(right.round_number || 0))
...
export const __testables = { creationRequestFingerprint, invitationCodeFor, invitationDigest, initialRoundBody, nextRoundBody, canAdoptPersistedRound, mutation }
